...
**Note**: The `bumpDependents` option is currently accepted in the configuration but dependency tracking automation is not yet implemented. You will need to manually ensure dependent components are bumped when their dependencies change, or use the `linked-versions` plugin if you want all components to share the same version.
